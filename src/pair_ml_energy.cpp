   ...
   for(int k=0;k<32;k++)
     ...
     */
        double fc_rij= fun_cutoff(rij,cutoff);
       	int pos2b = pos2b_fun(itype,jtype,6);
		...
		rho_values[i][k+pos2b+6] += lastp;		
           ...
           fvect_unit_Eg[k+18+pos2b] += Hx(mu_list[k]-rij)*pow(mu_list[k]-rij,3)*exp(-1.0*pow((rij-2.5)/cutoff,2))*fc_rij;
			
           // fvect_unit_Eg[k+16] += exp(-1.0*pow((cutoff-rij)/eta_list[k],2))*fc_rij;	            		
            //rho_values[i][k+8] += exp(-1.0*pow((rij-mu_list[k])/Rm,2))*fc_rij;			
               }
        //if(rij > 6.5) continue;
	...
	int ktype = bondk.type;
        ...
        double gcos1 = cos_theta2*(1-cos_theta2)*(1-3*cos_theta2);
        double gcos2 = pow((cos_theta2-0.25)*(cos_theta2-2.0/3),2);
        double gcos3 = (1-pow(cos_theta,15));		

        double G_cos1 =  1.0;
        double G_cos2 = (3*cos_theta2-1);
        double G_cos3 = cos_theta*(5*cos_theta2 -3);	

        int pos3b = pos3b_fun(itype,jtype,ktype,1)+36;		

        double prefactor=fkk_cut*exp(-1.2*rsij2/Rm_sq)*pow(rij*rik,-1.5); 
       fvect_unit_Eg[pos3b] += prefactor*gcos1;
       fvect_unit_Eg[pos3b+6] += prefactor*gcos2;
       fvect_unit_Eg[pos3b+12] += prefactor*gcos3;

       pos3b = pos3b+18;
       prefactor=fkk_cut*exp(-1.0*rsij2/cutoff2)*pow(rij*rik,-3);
       fvect_unit_Eg[pos3b] += prefactor*gcos1;
       fvect_unit_Eg[pos3b+6] += prefactor*gcos2;
       fvect_unit_Eg[pos3b+12] += prefactor*gcos3;

        pos3b = pos3b+18;		
        prefactor=fkk_cut*exp(-1.0*rvij2/Rm_sq)*pow(rij*rik,-1.0);			
        fvect_unit_Eg[pos3b] += prefactor*G_cos1;
        fvect_unit_Eg[pos3b+6] += prefactor*G_cos2;
        fvect_unit_Eg[pos3b+12] += prefactor*G_cos3;		


        pos3b = pos3b+18;
        prefactor=fkk_cut*exp(-0.8*rvij2)*pow(rij*rik,-2);
        fvect_unit_Eg[pos3b] += prefactor*G_cos1;
        fvect_unit_Eg[pos3b+6] += prefactor*G_cos2;
        fvect_unit_Eg[pos3b+12] += prefactor*G_cos3;	
        }		  
...
for(int k=0;k<6;k++)
 {	
     ...
     double lastp = rho_values[i][k];
     rho_values[i][k] = log(lastp);
     fvect_unit_Eg[k+108]= lastp*(rho_values[i][k]-1);

     double rho_sum1 = rho_values[i][k+6]+ 1.0;
     fvect_unit_Eg[k+114]= rho_sum1*(log(rho_sum1)-1.0)+1.0;

     double rho_sum2 = rho_values[i][k+18]+ 1.0;
     fvect_unit_Eg[k+120]= rho_sum2*(log(rho_sum2)-1.0)+1.0;


     double rho_sum3 = rho_values[i][k+12]+ 1.0;
     fvect_unit_Eg[k+126]= rho_sum3*(log(rho_sum3)-1.0)+1.0;

     rho_values[i][k+6] = log(rho_sum1);
     rho_values[i][k+12] = log(rho_sum2);
     rho_values[i][k+18] = log(rho_sum3);
		  }
...
//8-16 colum
         double gcos1 = cos_theta2*(1-cos_theta2)*(1-3*cos_theta2);
         double gcos1_dev = 2*cos_theta*(cos_theta2*(9*cos_theta2-8)+1);

//16-24 colum
         double gcos2 = pow((cos_theta2-0.25)*(cos_theta2-2.0/3),2);
         double gcos2_dev = 4*cos_theta*(cos_theta2-0.25)*(cos_theta2-2.0/3)*(2*cos_theta2 -11.0/12);
//24-36 colum	
         double gcos3 = (1-pow(cos_theta,15));
         double gcos3_dev = -15.0*pow(cos_theta,14);				 
				 

//8-16 colum
         double G1_cos = 1.0;
         double G1_cos_dev = 0.0;
...
//24-36 colum	
	...
	//double fjk_eta2 = fjk_cut/Rm_sq;
        double gausp= exp(-1.2*rsij2/Rm_sq)*pow(rij*rik,-1.5);
        double prefactor_gij = frjp_cut-(2.4*(rij+rik)/Rm_sq+1.5/rij)*fjk_cut;
        double prefactor_gik = frkp_cut-(2.4*(rik+rij)/Rm_sq+1.5/rik)*fjk_cut;
	...
	double fcos_g_factor = gcos_dev_sum*gausp;			
        ...
        pos3b = pos3b+18;			
        gausp= exp(-1.0*rsij2/cutoff2)*pow(rij*rik,-3);
        prefactor_gij = frjp_cut-(2.0*(rij+rik)/cutoff2+3.0/rij)*fjk_cut;
        prefactor_gik = frkp_cut-(2.0*(rik+rij)/cutoff2+3.0/rik)*fjk_cut;
        ...
        fik_sum += prefactor_gik*fr_g_factor;
            ...
            pos3b = pos3b+18;
            gausp= exp(-1.0*rvij2/Rm_sq)*pow(rij*rik,-1.0);
             double prefactor_Gij = frjp_cut-(2.0*(rij-rik)/Rm_sq+1.0/rij)*fjk_cut;
             double prefactor_Gik = frkp_cut-(2.0*(rik-rij)/Rm_sq+1.0/rik)*fjk_cut;
...
//16-24 colum	
	    ...
	    G_cos_dev_sum += gs3_factor*G3_cos_dev;			
            ...
            double fr_G1_factor = G_cos_sum*gausp;
	    double fcos_G1_factor = G_cos_dev_sum*gausp;			          

            fcos_factor_sum += fcos_G1_factor*fjk_cut;
	    fij_sum += prefactor_Gij*fr_G1_factor;
            fik_sum += prefactor_Gik*fr_G1_factor;

            pos3b = pos3b+18;
            gausp= exp(-0.8*rvij2)*pow(rij*rik,-2);
            prefactor_Gij = frjp_cut-(1.6*(rij-rik)+2.0/rij)*fjk_cut;
            prefactor_Gik = frkp_cut-(1.6*(rik-rij)+2.0/rik)*fjk_cut;
            ...
            double fr_G2_factor = G_cos_sum*gausp;
            double fcos_G2_factor = G_cos_dev_sum*gausp;

            fcos_factor_sum += fcos_G2_factor*fjk_cut;
            fij_sum += prefactor_Gij*fr_G2_factor;
            fik_sum += prefactor_Gik*fr_G2_factor;
 ...
 // Communicate U'(rho) values
  ...
  for(int ii = 0; ii < inum_full; ii++) {
	...
	int jtype = type[j];
	    ...
	    int pos2b = pos2b_fun(itype,jtype,6);
        int pos_pair_type = pos2b_fun(itype,jtype,1);		
        ...
        for(int k=0;k<6;k++)
	  {
	   ...
	   double pair_pot1_deriv = (fpair1_coeff+fcut_coeff)*exp(-1.0*pow(rij/eta_list[k],2));			
       fpair += fvect_dev[k+pos2b]*pair_pot1_deriv;
            ...
            fpair += fvect_dev[k+18+pos2b]*pair_pot2_deriv;
		
          double Urho1_prime_ij= 0.5*fvect_dev[k+108]*(rho_values[i][k]+rho_values[j][k]);
           fpair += Urho1_prime_ij*pair_pot1_deriv;
			
			
          double pair_SiSj = 0.0;
          double Urho2_prime_ij= 0.5*fvect_dev[k+114]*(rho_values[i][k+6]+rho_values[j][k+6]);
		   
		   if(pos_pair_type==0)
			   pair_SiSj = 1.0;
                   else 
			   pair_SiSj = 0.0;		   
           fpair += pair_SiSj*Urho2_prime_ij*pair_pot1_deriv;			

           double Urho3_prime_ij= 0.5*fvect_dev[k+120]*(rho_values[i][k+12]+rho_values[j][k+12]);
		   if(pos_pair_type==2)
			   pair_SiSj = 1.0;
                   else
			   pair_SiSj = 0.0;		   
           fpair += pair_SiSj*Urho3_prime_ij*pair_pot1_deriv; 			

 
           double Urho4_prime_ij= 0.5*fvect_dev[k+126]*(rho_values[i][k+18]+rho_values[j][k+18]);
                   if(pos_pair_type==1)
                           pair_SiSj = 1.0;
                   else
                           pair_SiSj = 0.0;
           fpair += pair_SiSj*Urho4_prime_ij*pair_pot1_deriv;
